...
for run_name in run_folders:
    ...
    print(f"   ✓ {run_name}: {count_after - count_before} imágenes cargadas")
...
data_to_save = {
    ...
    'runs_incluidos': [r for r in run_folders if os.path.isdir(os.path.join(base_data_dir, r))]
}
...
print(f"Runs combinados:   {', '.join(run_folders)}")
print(f"Total muestras:    {len(all_images)}")
