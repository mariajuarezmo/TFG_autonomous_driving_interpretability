...
print("=" * 70)
print("PREPROCESAMIENTO + ENTRENAMIENTO PILOTNET - VERSIÓN CON REBALANCEO")
...
print("\n[1/8] Cargando rutas de imágenes desde carpetas...")
...
print("\n[2/8] Analizando distribución de torques...")
...
# Estas dos variables sustituyen a max_torque_ref y min_torque_ref del original
...
config_file = os.path.join(output_dir, "preprocessing_config_rebalanceo.txt")
with open(config_file, "w") as f:
    f.write("CONFIGURACIÓN DEL PREPROCESAMIENTO (VERSIÓN COMBINADA + REBALANCEO)\n")
    f.write("=" * 70 + "\n\n")
...
# ========== ANÁLISIS DE ESQUIVAS ==========
print("\n[5/8] Analizando distribución para rebalanceo...")

esquiva_mask    = torch.abs(torques_real) >= ESQUIVA_THRESHOLD
no_esquiva_mask = ~esquiva_mask

num_esquivas    = esquiva_mask.sum().item()
num_no_esquivas = no_esquiva_mask.sum().item()

print(f"   Muestras totales:                {num_samples}")
print(f"   Muestras de esquiva (>={ESQUIVA_THRESHOLD:.1f}): {num_esquivas} ({num_esquivas/num_samples*100:.1f}%)")
print(f"   Muestras normales (<{ESQUIVA_THRESHOLD:.1f}):  {num_no_esquivas} ({num_no_esquivas/num_samples*100:.1f}%)")

esquiva_positiva = (torques_real >= ESQUIVA_THRESHOLD).sum().item()
esquiva_negativa = (torques_real <= -ESQUIVA_THRESHOLD).sum().item()
print(f"\n   Esquivas por dirección:")
print(f"      Positivas (derecha):    {esquiva_positiva}")
print(f"      Negativas (izquierda):  {esquiva_negativa}")

if DEBUG:
    plt.figure(figsize=(10, 4))
    plt.hist(torques_real.numpy(), bins=50, alpha=0.7, edgecolor='black')
    plt.axvline(x=ESQUIVA_THRESHOLD, color='r', linestyle='--', label=f'Umbral ({ESQUIVA_THRESHOLD:.1f})')
    plt.axvline(x=-ESQUIVA_THRESHOLD, color='r', linestyle='--')
    plt.xlabel("Torque"); plt.ylabel("Frecuencia")
    plt.title("Distribución de Torques"); plt.legend(); plt.grid(True, alpha=0.3)
    plt.savefig("distribucion_torques.png", dpi=150)
    plt.close()


# ========== PESOS PARA REBALANCEO ==========
print("\n[6/8] Creando pesos para rebalanceo...")
...
if REBALANCE:
    ...
    print(f"   Peso para no esquivas:         {sample_weights[no_esquiva_mask].mean():.3f}")
else:
    sample_weights = torch.ones(num_samples)
    print(f"   Rebalanceo desactivado, usando pesos uniformes")


# ========== DIVISIÓN TRAIN/TEST ==========
print("\n[7/8] Dividiendo datos en Train/Test...")
...
train_torques_real = denormalize(train_torques.cpu())
test_torques_real  = denormalize(test_torques.cpu())

train_esquivas = (torch.abs(train_torques_real) >= ESQUIVA_THRESHOLD).sum().item()
test_esquivas  = (torch.abs(test_torques_real)  >= ESQUIVA_THRESHOLD).sum().item()

print(f"\n   Distribución de esquivas:")
print(f"      Train: {train_esquivas} ({train_esquivas/len(train_idx)*100:.1f}%)")
print(f"      Test:  {test_esquivas} ({test_esquivas/len(test_idx)*100:.1f}%)")


# ========== MODELO ==========
print("\n[8/8] Construyendo y entrenando modelo PilotNet...")
...
def calcular_metricas(preds_norm, reales_norm):
    ...
    mae_esquiva_pos = torch.abs(preds_torque[esquiva_pos_mask] - reales_torque[esquiva_pos_mask]).mean().item() if esquiva_pos_mask.sum() > 0 else 0.0
    mae_esquiva_neg = torch.abs(preds_torque[esquiva_neg_mask] - reales_torque[esquiva_neg_mask]).mean().item() if esquiva_neg_mask.sum() > 0 else 0.0
...
train_loader = DataLoader(
    ...
    sampler=sampler if REBALANCE else None,
...
# Umbral en escala normalizada para la loss
esquiva_threshold_norm = (ESQUIVA_THRESHOLD - min_torque) / range_torque * 2 - 1
...
print(f"   ✓ Sampler activado: {REBALANCE}")
...
best_test_mae_esquiva = float('inf')
best_epoch            = 0
...
for epoch in range(num_epochs):
    ...
    # Guardar mejor modelo (basado en MAE de esquivas)
    if test_metrics['mae_esquiva'] < best_test_mae_esquiva:
        best_test_mae_esquiva = test_metrics['mae_esquiva']
        best_epoch = epoch + 1
        torch.save({
            'epoch': epoch,
            ...
            'best_mae_esquiva': best_test_mae_esquiva,
            'rebalanceo': {
...
print("-" * 70)
print(f"✓ Mejor modelo en época {best_epoch} con MAE esquivas = {best_test_mae_esquiva:.2f}")


# ========== ANÁLISIS FINAL ==========
print("\nAnálisis final...")

checkpoint = torch.load("pilotnet_rebalanceo_weights.pth")
...
test_no_esquiva_mask = ~test_esquiva_mask

print("\n" + "=" * 70)
print("MÉTRICAS FINALES - MODELO REBALANCEADO")
print("=" * 70)
...
if test_esquiva_mask.sum() > 0:
    mae_esquivas    = torch.abs(test_preds_torque[test_esquiva_mask]    - test_reales_torque[test_esquiva_mask]).mean().item()
    mae_no_esquivas = torch.abs(test_preds_torque[test_no_esquiva_mask] - test_reales_torque[test_no_esquiva_mask]).mean().item()
    print(f"MAE solo esquivas:       {mae_esquivas:.2f}")
    print(f"MAE sin esquivas:        {mae_no_esquivas:.2f}")

signos_correctos = ((test_preds_torque > 0) == (test_reales_torque > 0)).sum().item()
print(f"Signo correcto total:    {signos_correctos}/{len(test_preds_torque)} ({signos_correctos/len(test_preds_torque)*100:.1f}%)")
...
plt.xlabel("Epoch"); plt.ylabel("Error Relativo (%)")
plt.title("Error Relativo (mediana)"); plt.legend(); plt.grid(True, alpha=0.3)
...
plt.title(f"Predicciones vs Reales (Esquivas, n={len(esquivas_reales)})")
plt.grid(True, alpha=0.3); plt.axis('equal')

ax7 = plt.subplot(3, 4, 7)
if test_esquiva_mask.sum() > 0:
    errors_esquivas    = torch.abs(test_preds_torque[test_esquiva_mask]    - test_reales_torque[test_esquiva_mask])
    errors_no_esquivas = torch.abs(test_preds_torque[test_no_esquiva_mask] - test_reales_torque[test_no_esquiva_mask])
    plt.hist(errors_esquivas.numpy(),    bins=30, alpha=0.5, label='Esquivas',    edgecolor='red')
    plt.hist(errors_no_esquivas.numpy(), bins=30, alpha=0.5, label='No esquivas', edgecolor='blue')
    plt.xlabel("Error Absoluto"); plt.ylabel("Frecuencia")
    plt.title("Distribución de Errores por Tipo"); plt.legend(); plt.grid(True, alpha=0.3)

ax12 = plt.subplot(3, 4, 12)
ax12.axis('off')
textstr = f"""Modelo Rebalanceado:
-------------------
Epochs: {num_epochs}
Batch: {batch_size}
LR: {learning_rate}
...
Métricas Finales:
-------------------
MAE Total: {final_metrics['mae_torque']:.2f}
ER Total: {final_metrics['er_mean']:.2f}%
MAE Esquivas: {final_metrics['mae_esquiva']:.2f}
Signo Esquivas: {final_metrics['porcentaje_signo_esquiva']:.1f}%

Parámetros Rebalanceo:
-------------------
Activado: {REBALANCE}
Umbral: {ESQUIVA_THRESHOLD:.1f}
...
plt.tight_layout()
plt.savefig("pilotnet_rebalanceado_results.png", dpi=150)
print(f"   ✓ Gráfica: pilotnet_rebalanceado_results.png")
...
print(f"   ✓ Modelo: pilotnet_rebalanceo_weights.pth")

print("\n" + "=" * 70)
print("ENTRENAMIENTO CON REBALANCEO COMPLETADO")
print("=" * 70)
