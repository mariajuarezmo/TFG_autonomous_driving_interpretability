...
learning_rate = 1e-3
weight_decay = 1e-5
TORQUE_EPS = 5.0  # evita divisiones por valores casi 0
DEBUG = False
...
print("=" * 70)
print("ENTRENAMIENTO PILOTNET - VERSIÓN COMBINADA (Rebalanceo + SmoothL1)")
print("=" * 70)
...
print(f"   % esquivas reales: {esquiva_mask.float().mean().item() * 100:.2f}%")

# ANÁLISIS DE ESQUIVAS
print("\n[2/7] Analizando distribución para rebalanceo...")

# Identificar muestras de esquiva
esquiva_mask = torch.abs(torques_real) >= ESQUIVA_THRESHOLD
no_esquiva_mask = torch.abs(torques_real) < ESQUIVA_THRESHOLD

num_esquivas = esquiva_mask.sum().item()
num_no_esquivas = no_esquiva_mask.sum().item()

print(f"    Muestras totales: {num_samples}")
print(f"    Muestras de esquiva (>={ESQUIVA_THRESHOLD}): {num_esquivas} ({num_esquivas/num_samples*100:.1f}%)")
print(f"    Muestras normales (<{ESQUIVA_THRESHOLD}): {num_no_esquivas} ({num_no_esquivas/num_samples*100:.1f}%)")

# Distribución de esquivas por signo
...
# Histograma de torques
if DEBUG:
    plt.figure(figsize=(10, 4))
    plt.hist(torques_real.numpy(), bins=50, alpha=0.7, edgecolor='black')
    plt.axvline(x=ESQUIVA_THRESHOLD, color='r', linestyle='--', label=f'Umbral ({ESQUIVA_THRESHOLD})')
    plt.axvline(x=-ESQUIVA_THRESHOLD, color='r', linestyle='--')
    plt.xlabel("Torque")
    plt.ylabel("Frecuencia")
    plt.title("Distribución de Torques")
    plt.legend()
...
# CREACIÓN DE PESOS PARA REBALANCEO 
...
# DIVISIÓN TRAIN/TEST CON REBALANCEO
print("\n[4/7] Dividiendo datos en Train/Test con rebalanceo...")

# Primero dividimos los índices
...
# Análisis de distribución en train/test
train_torques_real = denormalize(train_torques.cpu(), max_torque_ref)
test_torques_real = denormalize(test_torques.cpu(), max_torque_ref)
...
#  MODELO 
print("\n[5/7] Construyendo modelo PilotNet...")
...
def calcular_metricas(preds_norm, reales_norm):
    ...
    if esquiva_mask.sum() > 0:
        ...
        porcentaje_signo_esquiva = signo_correcto_esquiva / esquiva_mask.sum().item() * 100
    else:
        mae_esquiva = 0.0
...
print("\n[6/7] Preparando entrenamiento con rebalanceo...")

# Crear WeightedRandomSampler para el train set
if REBALANCE:
    sampler = WeightedRandomSampler(
        weights=train_weights,
        num_samples=len(train_weights),
        replacement=True  # Permite muestreo con reemplazo para oversampling
    )
    shuffle_train = False  # El sampler ya maneja el muestreo aleatorio
else:
    sampler = None
    shuffle_train = True
...
class EsquivaSmoothL1Loss(nn.Module):
    ...
    def forward(self, preds, targets):
        errors = self.base_loss(preds, targets)
...
for epoch in range(num_epochs):
    ...
    # Mejor modelo (basado en error relativo y MAE de esquivas)
    if test_metrics['mae_esquiva'] < best_test_mae_esquiva:
        ...
        }, "pilotnet_combinado_weights.pth")
...
print(f" Mejor modelo en época {best_epoch}")
print(f"  MAE esquivas: {best_test_mae_esquiva:.2f}")

#  ANÁLISIS FINAL 
print("\n[8/7] Análisis final...")

checkpoint = torch.load("pilotnet_combinado_weights.pth")
...
# Calcular métricas separadas para esquivas y no esquivas
...
# 6. Distribución de errores (esquivas vs no esquivas)
...
ax12.axis('off')
textstr = f"""Modelo Combinado (Rebalanceo + SmoothL1):
-------------------
Epochs: {num_epochs}
Batch: {batch_size}
LR: {learning_rate}
...
plt.tight_layout()
plt.savefig("pilotnet_combinado_results.png", dpi=150)
print(f"    Gráfica: pilotnet_combinado_results.png")
...
print(f"   Modelo: pilotnet_combinado_weights.pth")

print("\n" + "=" * 70)
print("ENTRENAMIENTO COMBINADO COMPLETADO")
print("=" * 70)
